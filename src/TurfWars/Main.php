...
class Main extends PluginBase implements Listener
{
    ...
    public function onEnable()
    {
        ...
        if (!is_dir($this->getServer()->getDataPath() . "worlds/TW-1")) {
            $this->copymap(
                $this->getDataFolder() . "/maps/TW-BACKUP",
                $this->getServer()->getDataPath() . "/worlds/TW-1"
            );
            $this->getLogger()->info(
                "A TurfWars map was added to the worlds folder, you can add more by copy and rename it to TW-2, TW-3, TW-4..."
            );
        }
        if ($this->isDevelopmentBuild == true) {
            $this->getLogger()->error(
                "[ERROR] You cant use a development build... Disabling the plugin"
            );
            $this->getServer()
                ->getPluginManager()
                ->disablePlugin($this);
    ...
    public function SignUpdate()
    {
        ...
        if ($this->getServer()->isLevelLoaded($lobby->getFolderName())) {
            foreach ($lobby->getTiles() as $tile) {
                $signb = $this->getServer()
                    ->getDefaultLevel()
                    ->getBlock(new Vector3($tile->x, $tile->y, $tile->z));
                if (
                    $signb->getID() == 323 ||
                    $signb->getID() == 63 ||
                    $signb->getID() == 68
                    ...
                    if ($signt[0] == "§d[§9TW§d]") {
                        ...
                        $Status =
                            $this->games[$this->getGameByLevel($levelname)][
                                "Status"
                            ];
    ...
    public function onQuit(PlayerQuitEvent $event)
    {
        if ($this->inTurfWars($event->getPlayer())) {
            $s =
                $this->games[$this->getGameByPlayer($event->getPlayer())][
                    "Status"
                ];
            $this->LeaveCheck(
                $this->getGameByPlayer($event->getPlayer()),
    ...
    public function updateTerrain($game, $scorerteam)
    {
        $x =
            570 +
            $this->games[$game]["RedScore"] -
            $this->games[$game]["BlueScore"];
    ...
    {
        $gameplayers = $this->getServer()
            ->getLevelByName($this->games[$game]["Arena"])
            ->getPlayers();
        ...
        if (
            ...
            if (count($gameplayers) < 3 || $blue == 0 || $red == 0) {
                ...
                foreach ($gameplayers as $player) {
                    ...
                    $player->sendPopup(" ");
                    $player->sendMessage(
                        $this->PREFIX . "Game was cancelled, to less players."
                    );
                }
                $this->deleteDirectory(
                    $this->getServer()->getDataPath() .
                        "/worlds/" .
                        $this->games[$game]["Arena"]
                );
                $this->copymap(
                    $this->getDataFolder() . "/maps/TW-BACKUP",
                    $this->getServer()->getDataPath() .
                ...
                if ($signt[0] == "§d[§9TW§d]") {
                    ...
                    if ($signt[2] != "§fJoinable") {
                        $event
                            ->getPlayer()
                            ->sendMessage(
                                $this->PREFIX . "The game is already full."
                            );
                        return;
                    }
                    ...
                    if ($this->getServer()->isLevelGenerated($signt[1])) {
                        ...
                        $event
                            ->getPlayer()
                            ->sendMessage(
                                $this->PREFIX . "This game is not in usage."
                            );
                    }
                }
            }
    ...
    {
        if (
            $this->inTurfWars($player = $event->getEntity()->getOwiningEntity())
        ) {
            if (
                $this->games[$this->getGameByPlayer($player)]["Status"] !=
                "INGAME"
            ) {
                ...
                if ($this->config->get("Enable_PC_Arrow_Mechanics")) {
                    $event
                        ...
                        ->setMotion(
                            new Vector3(
                                -\sin(($player->yaw / 180) * M_PI) *
                                    \cos(($player->pitch / 180) * M_PI),
                                -\sin(($player->pitch / 180) * M_PI),
                                \cos(($player->yaw / 180) * M_PI) *
                                    \cos(($player->pitch / 180) * M_PI)
                            )
                        );
                    $s = 3.75;
                    $event->getEntity()->setMotion(
                        $event
                            ->getEntity()
                            ->getMotion()
                            ->multiply($s)
    ...
    public function onBreak(BlockBreakEvent $event)
    {
        ...
        if (
            ...
            if ($tile->getText()[0] == "§d[§9TW§d]") {
                $event
                    ->getPlayer()
                    ->sendMessage($this->PREFIX . "Join sign was removed.");
            }
        }
    }
        ...
        if ($this->inTurfWars($player = $event->getPlayer())) {
            if (
                !(
                    ...
                    $this->games[$this->getGameByPlayer($player)]["Status"] ==
                        "JOINABLE" ||
                    $this->games[$this->getGameByPlayer($player)]["Status"] > 5
                )
            ) {
                $event->setCancelled();
            }
            $block = $event
                ->getPlayer()
                ->getLevel()
                ->getBlock(new Vector3($player->x, 63, $player->z));
            if ($this->getTeam($player) == "Red") {
            ...
            if (
                ...
                if ($this->inTurfWars($victim)) {
                    ...
                    if (!$killer instanceof Arrow) {
                        if (
                            $this->getTeam($killer) == $this->getTeam($victim)
                        ) {
                            $event->setCancelled();
                            return;
                        }
    ...
    public function joinGame($player, $levelname)
    {
        ...
        if (
            $this->games[$this->getGameByLevel($levelname)]["Status"] ==
                "JOINABLE" ||
            $this->games[$this->getGameByLevel($levelname)]["Status"] > 5
        ) {
    ...
    public function GameSetup($player, $Arena)
    {
        ...
        if (count($Players) == 0) {
            if ($this->getServer()->isLevelLoaded($Arena)) {
                $this->getServer()->unloadLevel(
                    $this->getServer()->getLevelByName($Arena)
                );
            }
        }
        $this->getServer()->loadLevel($Arena);
        ...
        $player->sendMessage(
            $this->PREFIX .
                "Bow and Arrows were added to your inventory, open it to select them."
        );
    ...
    {
        /* Game1, Game2 etc...*/ $levelname = $this->games[$game]["Arena"];
        $level = $this->getServer()->getLevelByName($levelname);
        if ($team == "Red") {
            $this->games[$game]["RedScore"] =
                $this->games[$game]["RedScore"] + 1;
        } elseif ($team == "Blue") {
            $this->games[$game]["BlueScore"] =
                $this->games[$game]["BlueScore"] + 1;
        }
        if (
            $this->games[$game]["RedScore"] -
                $this->games[$game]["BlueScore"] ==
        ...
        if (
            ...
            foreach (
                ...
                $player->sendMessage(
                    $this->PREFIX . " You got 4 coins for participation."
                );
                $this->getServer()
                    ->getPluginManager()
                    ->getPlugin("Auth")
    ...
    public function Second()
    {
        ...
        foreach ($this->games as $game => $value) {
            ...
            if ($this->getServer()->isLevelLoaded($Arena)) {
                ...
                if (
                    $this->games[$this->getGameByLevel($Arena)]["Status"] ==
                    "INGAME"
                ) {
                    foreach (
                        $this->getServer()
                    ...
                    foreach ($level->getPlayers() as $player) {
                        $player->sendPopup(
                            "§7Starts in " . $c . gmdate("i.s", $Status - 5)
                        );
                    }
                    ...
                    foreach ($level->getPlayers() as $player) {
                        ...
                        $player->sendMessage(
                            $this->PREFIX . "Game has started! Happy fighting!"
                        );
                    }
                }
            }
            ...
            foreach ($files as $file) {
                if ($file->isDir()) {
                    ...
                    if (is_dir($dirname . DIRECTORY_SEPARATOR . "region")) {
                        if (
                            $handle = opendir(
                                $dirname . DIRECTORY_SEPARATOR . "region"
                            )
                        ) {
